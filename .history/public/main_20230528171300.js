...
//     deleteButton.addEventListener("click", () => {
//       wt.deleteEntry(entry.ID); // Delete the entry from the tracker
//       updateEntryList(wt.entries); // Update the entry list
//     });
...
class MusicTracker {
  ...
  static rowHtml() {
    return `
      <tr class="tracker__row">
          <td>
              <span class="tracker__ID"></span>
          </td>
          <td>
              <span class="tracker__song"></span>
          </td>
          <td>
              <span class="tracker__bpm"></span>
          </td>
          <td>
              <span class="tracker__date"></span>
          </td>
          <td>
              <button class="tracker__button">X</button>
          </td>
  ...
  createEntryRow(entry) {
    ...
    const idSpan = row.querySelector(".tracker__ID");
    const songSpan = row.querySelector(".tracker__song");
    const bpmSpan = row.querySelector(".tracker__bpm");
    const dateSpan = row.querySelector(".tracker__date");
    const deleteButton = row.querySelector(".tracker__button");

    idSpan.textContent = entry.ID;
    songSpan.textContent = entry.song;
    bpmSpan.textContent = entry.bpm;
...
function updateEntryList(entries) {
  ...
  entries.forEach((entry) => {
    ...
    deleteButton.classList.add("button"); // Apply the "button" class
    deleteButton.addEventListener("click", () => {
      wt.deleteEntry(entry.ID); // Delete the entry from the tracker
      updateEntryList(wt.entries); // Update the entry list
    });
