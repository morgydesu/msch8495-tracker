...
class MusicTracker {
  ...
  getNextPlaylistItemID() {
    return this.playlistItems.length + 1;
  }
  ...
  static html() {
    return `
      ...
      <div class="playlist__container">
        <div class="playlist__title">Playlist</div>
  ...
  static playlistItemHtml(playlistItem) {
    return `
      <div class="playlist__item">
        <div class="playlist__item-id">${playlistItem.ID}</div>
        <div class="playlist__item-details">
          <div class="playlist__item-song">${playlistItem.song}</div>
          <div class="playlist__item-artist">${playlistItem.artist}</div>
        </div>
      </div>
    `;
  }
  ...
  updateView() {
    const tableBody = this.root.querySelector(".tracker__entries");
    const playlistContainer = this.root.querySelector(".playlist__items");

    tableBody.querySelectorAll(".tracker__row").forEach((row) => {
      row.remove();
    });

    playlistContainer.querySelectorAll(".playlist__item").forEach((item) => {
      item.remove();
    });

    this.entries.forEach((entry, index) => {
      const template = document.createElement("template");
      template.innerHTML = MusicTracker.rowHtml({ ...entry, ID: index + 1 }).trim();
      const newRow = template.content.firstElementChild;
      newRow.dataset.id = index + 1;
      tableBody.appendChild(newRow);

      const playlistItem = { ...entry, ID: index + 1 };
      this.playlistItems.push(playlistItem);
      const playlistItemTemplate = document.createElement("template");
      playlistItemTemplate.innerHTML = MusicTracker.playlistItemHtml(playlistItem).trim();
      const newPlaylistItem = playlistItemTemplate.content.firstElementChild;
  ...
  deleteEntry(idToDelete) {
    ...
    this.playlistItems = this.playlistItems.filter((item) => item.ID !== idToDelete);
    this.saveEntries();
    this.updateView();
    const row = this.root.querySelector(`.tracker__row[data-id="${idToDelete}"]`);
    const playlistItem = this.root.querySelector(`.playlist__item-id:contains(${idToDelete})`).closest(".playlist__item");
    if (row) {
      row.remove();
    }
    if (playlistItem) {
  ...
  updatePlaylistIDs() {
    const playlistItems = this.root.querySelectorAll(".playlist__item-id");
  ...
  filterEntries(searchTerm) {
    ...
    this.updateFilteredView(filteredEntries);
  }

  updateFilteredView(filteredEntries) {
    const tableBody = this.root.querySelector(".tracker__entries");
