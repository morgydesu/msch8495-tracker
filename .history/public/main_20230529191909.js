...
class MusicTracker {
  ...
  constructor(root) {
    ...
    this.root.querySelector(".entry-list").addEventListener("click", (event) => {
...
this.root.querySelector(".discover__albums").addEventListener("click", (event) => {
  ...
  // Check if the clicked element is an album image, album title, or album artist
  if (
    target.classList.contains("album__image") ||
    target.classList.contains("album__title") ||
    target.classList.contains("album__artist")
  ) {
    // Get the album information
    const albumImage = target.parentElement.querySelector(".album__image")
      .src;
    const albumTitle = target.parentElement.querySelector(".album__title")
      .textContent;
    const albumArtist = target.parentElement.querySelector(".album__artist")
      .textContent;

    // Call the showPopup function and pass the album information
    this.showPopup(albumImage, albumTitle, albumArtist);
  }
});
