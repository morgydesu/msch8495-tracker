...
class MusicTracker {
  ...
  constructor(root) {
    ...
    this.root.querySelector(".tracker__add").addEventListener("click", () => {
      ...
      const newEntry = {
        ID: this.getNextID(),
        song: this.root.querySelector(".tracker__song").value,
        bpm: this.root.querySelector(".tracker__BPM", ".tracker__bpm-image").value,
        // image: this.root.querySelector(".tracker__bpm-image").value,
        date: `${year}-${month}-${day}`,
      };
  ...
  updateView() {
    ...
    this.entries.forEach((entry) => {
      const listItem = document.createElement("div");
      listItem.textContent = `ID: ${entry.ID}, Song: ${entry.song}, BPM: ${entry.bpm}, Image: ${entry.image}, Date: ${entry.date}`;
