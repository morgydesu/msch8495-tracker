...
class MusicTracker {
  ...
  updateView() {
    ...
    this.entries.forEach((entry) => {
      ...
      const deleteButton = document.createElement("button");
deleteButton.innerHTML = "&#xe872;"; // Use innerHTML instead of textContent to render the HTML entity
deleteButton.classList.add("delete-button"); // Optionally, add a class for further styling
deleteButton.addEventListener("click", () => {
  this.deleteEntry(entry.ID);
});
